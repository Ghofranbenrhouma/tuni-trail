...
export default function Modal({ open, initialView, onClose, onToast }) {
  ...
  useEffect(() => {
    if (open && initialView) { setView(initialView); setErr('') }
  }, [open, initialView])
  ...
  const handleFileSelect = (file) => {
    ...
    reader.onload = (e) => { setForm(f => ({ ...f, document: e.target.result, documentName: file.name })); setErr('') }
    reader.readAsDataURL(file)
  }

  const handleDrop = (e) => { e.preventDefault(); setDragOver(false); handleFileSelect(e.dataTransfer.files[0]) }

  // ── Org submit: register then submit org request (both async) ─────────
  const handleOrgSubmit = async () => {
    ...
    setSubmitting(true)
    // 1) Create account (role pending_org is set by backend after org request)
    ...
    // 2) Submit org request
    const orgRes = await submitOrgRequest({
      ...
      document_data: form.document,
    })
    setSubmitting(false)
    if (orgRes && orgRes.error) { setErr(orgRes.error); return }
  ...
  const handleDemo = async (role) => {
    ...
    if (!res.error) { onClose(); onToast && onToast(`Bienvenue ${role === 'org' ? 'Tribus Aventure' : 'Ahmed'} !`) }
    else onToast && onToast('⚠️ Compte démo indisponible — vérifiez le backend')
  }
      ...
      <div className={`modal-box${view === 'orgForm' ? ' modal-box-wide' : ''}`}>
        ...
        {view === 'login' && (
          <>
            ...
            <button className="btn-mfull" onClick={handleLogin} disabled={submitting}>
        ...
        {view === 'register' && (
          <>
            ...
            <button className="btn-mfull" onClick={handleRegister} disabled={submitting}>
              {submitting ? '⏳ Création...' : (form.role === 'org' ? 'Continuer → Formulaire organisateur' : 'Créer mon compte')}
            </button>
        ...
        {view === 'orgForm' && (
          <>
            ...
            <div className="org-form-scroll">
              ...
              <textarea className="m-inp m-textarea"
              ...
              <textarea className="m-inp m-textarea"
                ...
                value={form.eventHistory} onChange={e => set('eventHistory', e.target.value)}
                style={{ minHeight: 60 }} />
            </div>
